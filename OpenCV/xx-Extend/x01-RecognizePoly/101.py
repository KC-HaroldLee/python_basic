...
def setLabel (image, str, contour):
    (text_width, text_height), baseline = cv.getTextSize(str, cv.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.7, 1)

    # 간단!
    x,y, w,h = cv.boundingRect(contour)

    pt_x = x+int((w-text_width)/2)
    pt_y = y+int((h +text_height)/2)
...
os.chdir('xx-Extend\\x01-RecognizePoly\\images')
img_color = cv.imread('test1.png', cv.IMREAD_COLOR)
